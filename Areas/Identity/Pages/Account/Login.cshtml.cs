...
namespace PinewoodGrow.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class LoginModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            // Aidan Shaefer - Clear userName cookie
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    var vol = _context.Volunteers.Where(e => e.Email == Input.Email).FirstOrDefault();
                    CookieHelper.CookieSet(HttpContext, "userName", vol.FullName, 3200);
                    if (String.IsNullOrEmpty(vol.Phone))
                    {
                        //Nag to complete the profile?
                        TempData["message"] = "Please enter the phone number.";
                        returnUrl = "~/VolunteerAccount/Edit";
                    }
                ...
                {
                    var vol = _context.Volunteers.Where(e => e.Email == Input.Email).FirstOrDefault();
                    if (vol == null) //check if they are in the system
                    {
                        string msg = "Error: Account for " + Input.Email + " has not been created by the Admin.";
                        ModelState.AddModelError(string.Empty, msg);
                    }
                    else if (!vol.Active) //check if they are active
                    {
